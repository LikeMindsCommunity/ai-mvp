  ...
  Widget userTileBuilder(
    ...
    return ListTile(
      leading: LMChatProfilePicture(
        imageUrl: user.imageUrl, // Use the user data for the profile picture
      ...
      subtitle: Text(
        user.customTitle ?? 'No custom title',
      ), // Display custom title if available
      trailing: const Icon(
        Icons.star,
        color: Colors.amber,
      ), // Add a custom trailing icon
      onTap: () {
...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('LikeMinds Demo')),
      body: Center(
        child: ElevatedButton(
          onPressed: () async {
            ...
            final response = await LMChatCore.instance.showChatWithApiKey(
              ...
              uuid:
                  "test-user-unique-id-participant", // TODO: Replace with a unique user ID
            );
